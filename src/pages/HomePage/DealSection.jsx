...
const DealsSection = ({ dealProducts }) => {
  const targetTime = moment()
    .add(1, "day")
    .set({ hour: 17, minute: 0, second: 0, millisecond: 0 });
  /* Get current date ==> add 1 day, 17pm  */
  const dealOfTheDayProduct = dealProducts?.[0] || {};
  const dealPath = PATHS.PRODUCTS + `/${dealOfTheDayProduct.slug}`;
  return (
            ...
            >
              ...
              <div className="deal-content">
                <h3 className="product-title">
                  <Link to={dealPath}>{dealOfTheDayProduct.name}</Link>
                </h3>
                ...
                <Link to={dealPath} className="btn btn-link">
                  <span>Shop Now</span>
